...
export async function PUT(request: NextRequest, context: RouteContext) {
    ...
    const response = await caribApiFetch(`/admin/press-releases/${encodeURIComponent(id)}/active`, {
        method: "PUT",
        headers: authHeader,
    ...
    if (!response.ok) {
        return Response.json(
            { error: typeof payload?.message === "string" ? payload.message : "Unable to update release visibility." },
